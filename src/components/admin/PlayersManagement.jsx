...
export default function PlayersManagement() {
  ...
  const fetchList = useCallback(async () => {
    try {
      const { data } = await axios.get("/admin/players");
  ...
  const buildFormData = () => {
    ...
    fd.append("weightClass", form.weightClass.trim());
    fd.append("clubName", form.clubName.trim());
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`/admin/players/${editingId}`, fd, {
        ...
        await axios.post("/admin/players", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = (p) => {
    if (!window.confirm(`Delete player "${p.name}"? This cannot be undone.`))
      return;
    axios
      .delete(`/admin/players/${p._id}`)
      .then(() => {
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl border-l border-gray-200 flex flex-col">
            ...
            >
              ...
              <div>
                ...
                />
                {(photoPreview || (editingId && thumbSrc(items.find((x) => x._id === editingId)?.photo))) ? (
                  <img
                    src={photoPreview || thumbSrc(items.find((x) => x._id === editingId)?.photo)}
                    alt=""
                    loading="lazy"
              ...
              <div>
                ...
                <input
                  value={form.clubName}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, clubName: e.target.value }))
                  }
                  className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm"
                />
              </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium">Active (show on public site)</span>
              </label>
