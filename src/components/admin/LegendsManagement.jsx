...
export default function LegendsManagement() {
  ...
  const fetchList = useCallback(async () => {
    try {
      const { data } = await axios.get("/admin/legends");
      setItems(Array.isArray(data?.data) ? data.data : []);
    } catch (e) {
      toast.error(e?.response?.data?.message || "Failed to load legends");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = (l) => {
    ...
    setForm({
      ...
      achievements:
        Array.isArray(l.achievements) && l.achievements.length
          ? [...l.achievements]
          : [""],
      bio: l.bio || "",
      notableTitles: l.notableTitles || "",
  ...
  const buildFormData = () => {
    ...
    fd.append("yearsActive", form.yearsActive.trim());
    fd.append("achievements", JSON.stringify(ach));
    fd.append("bio", form.bio);
    fd.append("notableTitles", form.notableTitles.trim());
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`/admin/legends/${editingId}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Legend updated");
      } else {
        await axios.post("/admin/legends", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = (l) => {
    if (!window.confirm(`Delete legend "${l.name}"?`)) return;
    axios
      .delete(`/admin/legends/${l._id}`)
      .then(() => {
    ...
    <div className="p-6 relative">
      ...
      {loading ? (
        ...
        <>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-3 py-3 text-left hidden sm:table-cell">Era</th>
                  <th className="px-3 py-3 text-left hidden md:table-cell">Role / Titles</th>
              ...
              <tbody>
                {pageRows.map((l) => (
                  <tr key={l._id} className="border-b border-gray-100">
                    ...
                    <td className="px-3 py-2 text-gray-600 hidden md:table-cell max-w-[200px] truncate">
                      {l.notableTitles || "—"}
                    </td>
          ...
          {filtered.length === 0 ? (
            <p className="text-center text-gray-500 py-8">No legends found.</p>
          ) : (
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl border-l border-gray-200 flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <h3 className="text-lg font-bold">
                {editingId ? "Edit Legend" : "Add Legend"}
              </h3>
              <button
                type="button"
                onClick={closePanel}
            ...
            >
              ...
              <div>
                ...
                <input
                  value={form.notableTitles}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, notableTitles: e.target.value }))
                  }
                  className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm"
                />
              </div>
              <label className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">
                  Featured (prominent on Legends page)
                </span>
              </label>
